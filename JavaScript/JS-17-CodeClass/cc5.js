...
ogrenciNotlarList.map((a) => {
    const yuksekNot = Math.max(...a.notes);
    enYüksekNot.push({name: a.name, enYüksekNot: yuksekNot});
})
console.log(enYüksekNot);


//? b- Öğrencisinin adını ve aldığı en düşük notları yeni bir listeye almak istiyor

const enDusukNotListesi=({name, notes})=>{
    return {
        name:name,
...
const enDusuknot=ogrenciNotlarList.map((x)=>enDusukNotListesi(x))
...
const notOrtalamasi=({name,notes})=>{
    const notOrt=(notes.reduce((acc,curr)=>(acc+curr))/notes.length).toFixed(1);
    return {
        name:name,
        notes:notOrt
    }
}
const ogrenciNotOrtalamari=ogrenciNotlarList.map((a)=>notOrtalamasi(a))
console.log(ogrenciNotOrtalamari);

//? d- Notları 70 ve 70den büyüklere 10 puan, 70 den az olanlara 5 puan kanaat notu verecek. notu 100den büyükse 100den büyük olamayacağı için 100 olarak geçecek
...
const newOrt = ({name, notes}) => {
    let oldOrt=Number((notes.reduce((acc,curr)=>(acc+curr))/notes.length).toFixed(1));
    (oldOrt>70)?oldOrt+=10:oldOrt+=5; 
    (oldOrt>100)?oldOrt=100:oldOrt;
    return {
        name:name,
        notes:oldOrt
    }
  }
const kanaatliNot=ogrenciNotlarList.map((a)=>newOrt(a));
console.log(kanaatliNot);
...
const yeniListe=customerList.map((a)=>duzenle(a))
