...
--SELECT * FROM Customer LIMIT 0,10;--Limit:kaçıncı kayıttan sonra, kaç adet kayıt gelsin.
--SELECT * FROM Customer LIMIT 10;-- Başlangıçtan default:0
...
-----------------JOINS----------------
--Birden fazla tablodaki kayıtları tek bir tabloda getirmek için kullanılıyor.

--* INNER JOIN --yanlızca kesişen kayıtları getirir.
--* DEFAULT JOIN yöntemidir. Sadece JOIN olarak da yazılabili. Piyasa standartı INNER JOIN olarak yazılır.
...
*/
/*
SELECT c.FirstName, c.LastName, c.City, c.Country, i.InvoiceDate, i.Total
FROM Customer AS c
...
-- LEFT JOIN -- Üst tablodaki (LEFT) bütün kayıtları getirmek şartıyla JOIN tablodaki (RIGHT) eşleşmiş verileri de getir.
...
-- RIGHT JOIN -- JOIN tablodaki (RIGHT) bütün kayıtları getirmek şartıyla Üst tablodaki (LEFT) eşleşmiş verileri de getir.
...
--SELECT COUNT (CustomerId) AS kayitSayisi FROM Customer; -- Herhangi bir sütun ismi kullanmalıyız (PRIMARY)
--SELECT COUNT(CustormerId) AS kayitSayisi FROM Customer WHERE Country = 'USA';
...
--SELECT COUNT(InvoiceId) AS adet, ROUND(AVG(Total)) AS ortlamaTutar FROM Invoice WHERE BillingCountry IN ('USA', 'Canada');-- default 1 karakter.
...
--* LENGTH -- Karakter sayısı (Kayıt sayısımı tek satıra düşürmez.)
